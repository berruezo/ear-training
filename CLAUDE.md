...
- **Hint system** — `#hint-container` sits between `.game-controls` and `#interval-answers` in `view-game`. It appears only after the user manually presses Play a second time for the current group (`g.playCount >= 2`) and disappears once a guess is made. Clicking `#hint-btn` toggles `#hint-menu` (dropdown). Menu items: **Slow replay** (`#hint-slow`, intervals + scales only) and **Play arpeggiated** (`#hint-arpeggiate`, chords only) — both fetch `/hint` endpoint and play the result; **Root position** (`#hint-root`, chords only, visible only when `gameState.chordInversions` is true) — replays the chord in root position simultaneously using `g.rootNotes`; **Song mnemonic** (`#hint-song`, intervals only, hidden for unison class 0) — fetches `/hint?type=interval_song` which plays a well-known song starting from the exact root note at the correct transposition, returns `X-Song-Name` header; the song name is shown in `#hint-song-label` (a `.hint-song-label` span inside `#hint-container`) while the audio plays and hidden afterward; **Eliminate an option** (`#hint-eliminate`, all exercises) removes one random non-eliminated wrong answer, pushing it to `g.eliminated[]`, and disables itself when only 2 options remain. `updateHintState()` is called at the end of `updateAllButtons()` to keep visibility in sync. CSS rule `.hint-container[hidden], .hint-item[hidden] { display: none }` is required — without it the `hidden` attribute is overridden by the `display: flex/block` rules on those elements (author CSS wins over UA stylesheet). `g.playCount` is incremented at the top of `playCurrentGroup()` (not in the `play` audio event) because headless Chromium may not fire the `play` event. `stopHintAudio()` is a function declaration (hoisted) so `playCurrentGroup()` can call it even though the hint code appears later in the file.
- **`/hint` endpoint** — `GET /hint?type=interval|chord|scale|interval_song&notes=<csv-midi>&tempo=<bpm>&simultaneous=0|1`. Returns WAV audio. Chords (simultaneous=0, default): sequential at 80 BPM. Chords (simultaneous=1): played simultaneously at 1.0s duration (root position replay). Intervals/scales: sequential at half the current tempo (min 40 BPM). `interval_song`: plays a well-known song mnemonic transposed to start from `notes[0]`, using `render_melody_wav()` with per-note durations from `INTERVAL_SONG_MNEMONICS`; responds with `X-Song-Name` header (e.g., "Star Wars"). Returns 400 on bad input, 404 if no mnemonic exists (e.g., unison). `eliminated` values in `g.eliminated[]` are always strings (interval class numbers, chord names, scale names) to match `btn.dataset.*`.
- **`INTERVAL_SONG_MNEMONICS`** — dict in `main.py` keyed by `(interval_class, 'asc'|'desc')` (class 1–12). Each value is `(display_name, [(semitone_offset, duration_beats), ...], bpm)`. Root is always `notes[0]` (lower note for ascending, upper note for descending). Offsets are transposed to MIDI absolute values with octave-clamping. The `render_melody_wav(sequence, bpm)` function renders the melody with per-note durations via the fluidsynth lock. Song choices can be refined independently — the data table is self-contained in `INTERVAL_SONG_MNEMONICS`.
- **`/chord/exercise` root notes** — always sends `X-Root-Notes` header with the root-position MIDI notes (saved before `apply_chord_inversion()` is called). Frontend stores as `g.rootNotes`. When inversions are disabled, `X-Root-Notes` equals `X-Notes`. `g.rootNotes` is used by the Root position hint.
